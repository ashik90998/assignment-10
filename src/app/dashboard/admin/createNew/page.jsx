...
export default function CreateNewRequestPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      router.push(redirects[user?.role] || "/dashboard/admin/myRequests");
    } catch (err) {
      alert(err.message);
    } finally {
      setLoading(false);
      ...
      <Card className="p-6 bg-white dark:bg-[#0B1F3A] border shadow-md">
        <Form onSubmit={handleSubmit} className="space-y-5">
          ...
          <TextField isRequired>
            ...
            <Input isDisabled={isBlocked} value={formData.requestMessage} onChange={(e) => setFormData({ ...formData, requestMessage: e.target.value })} />
          </TextField>
